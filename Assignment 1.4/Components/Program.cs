...
namespace Components
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            DFlipFlopGate dFlipFlopGate = new DFlipFlopGate();
            if (!dFlipFlopGate.TestGate())
                Console.WriteLine("bug at DFlipFlopGate");
            else
                Console.WriteLine("DFlipFlopGate is clear");

            SingleBitRegister singleBitRegister = new SingleBitRegister();
            if (!singleBitRegister.TestGate())
                Console.WriteLine("bug at SingleBitRegister");
            else
                Console.WriteLine("SingleBitRegister is clear");

            MultiBitRegister multiBitRegister = new MultiBitRegister(7);
            if (!multiBitRegister.TestGate())
                Console.WriteLine("bug at MultiBitRegister");
            else
                Console.WriteLine("MultiBitRegister is clear");

            Memory memory = new Memory(2,3);
            if (!memory.TestGate())
                Console.WriteLine("bug at memory");
            else
                Console.WriteLine("memory is clear");

            Counter counter = new Counter(4);
            if (!counter.TestGate())
                Console.WriteLine("bug at counter");
            else
                Console.WriteLine("counter is clear");
            ...
            //if (BitwiseMultiwayDemux.TestGate())
                //WriteLine("corrupt bug at bitwiseMultiwayMux gate");
            if (halfAdder.TestGate())
            ...
            if (aLU.TestGate())
                Console.WriteLine("corrupt bug at ALU gate");
            if (!dFlipFlopGate.TestGate())
                Console.WriteLine("corrupt bug at dFlipFlopGate gate");
            if (singleBitRegister.TestGate())
                Console.WriteLine("corrupt bug at dFlipFlopGate gate");
            if (multiBitRegister.TestGate())
                Console.WriteLine("corrupt bug at dFlipFlopGate gate");
